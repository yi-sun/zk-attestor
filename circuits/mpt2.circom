...
template MPTInclusionFixedKeyHexLen2(maxDepth, keyHexLen, maxValueHexLen) {
    ...
    component leaf = LeafCheck2(keyHexLen, maxValueHexLen);    
	...
	exts[layer] = ExtensionCheck2(keyHexLen, 64);	
...
template MPTInclusionNoBranchTermination2(maxDepth, maxKeyHexLen, maxValueHexLen) {
    ...
    component leaf = LeafCheck2(maxKeyHexLen, maxValueHexLen);
    leaf.keyNibbleHexLen <== leafSelector.outLen;
    for (var idx = 0; idx < maxKeyHexLen; idx++) {
	leaf.keyNibbleHexs[idx] <== leafSelector.out[idx];
    }
	...
	exts[layer] = ExtensionCheck2(maxKeyHexLen, 64);	
	exts[layer].keyNibbleHexLen <== keyFragmentStarts[layer + 1] - keyFragmentStarts[layer];
	for (var idx = 0; idx < maxKeyHexLen; idx++) {
	    exts[layer].keyNibbleHexs[idx] <== extKeySelectors[layer].out[idx];
	}
