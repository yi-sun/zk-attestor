...
template RlpArrayCheck(maxHexLen, nFields, arrayPrefixMaxHexLen, fieldMinHexLen, fieldMaxHexLen) {
    ...
    signal input arrayRlpPrefix1HexLen;    
    signal input fieldRlpPrefix1HexLen[nFields];
    ...
    component rlpArrayPrefix1_eq = IsEqual();
    rlpArrayPrefix1_eq.in[0] <== arrayRlpPrefix1HexLen;
    rlpArrayPrefix1_eq.in[1] <== rlpArrayPrefix.isBig * rlpArrayPrefix.prefixOrTotalHexLen;
    ...
    component prefixChecks[nFields];
	...
	fieldPrefix[idx].in[1] <== shiftToFieldRlps[idx].out[1];
        ...
        prefixChecks[idx] = IsEqual();
	prefixChecks[idx].in[0] <== fieldRlpPrefix1HexLen[idx];
	prefixChecks[idx].in[1] <== fieldPrefix[idx].isBig * fieldPrefix[idx].prefixOrTotalHexLen;
    ...
    var check = rlpArrayPrefix.isValid + rlpArrayPrefix1_eq.out;
    for (var idx = 0; idx < nFields; idx++) {
    	check = check + fieldPrefix[idx].isValid + prefixChecks[idx].out;
    }
...
// fieldMinHexLens, fieldMaxHexLens are arrays of length nFields
template RlpArrayCheckNoPrefix(maxHexLen, nFields, arrayPrefixMaxHexLen, fieldMinHexLen, fieldMaxHexLen) {
    signal input in[maxHexLen];
