...
BUILD_DIR=../../build/eth_addr_storage
CIRCUIT_NAME=eth_addr_storage
...
start=`date +%s`
npx snarkjs zkey export soliditycalldata "$BUILD_DIR"/public.json "$BUILD_DIR"/proof.json > "$BUILD_DIR"/solidity_input.in
end=`date +%s`
echo "DONE ($((end-start))s)"
